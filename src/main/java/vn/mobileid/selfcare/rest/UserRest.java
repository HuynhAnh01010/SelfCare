...
public class UserRest {
    ...
    public UserRest(CallApiRsspService callApiRsspService, SelfCareProperties selfCareProperties, AuthenticationManager authenticationManager) {
    ...
    @PostMapping("/tse/owner/changeEmail")
    public ResponseEntity ownerChangeEmail(HttpServletResponse response, HttpServletRequest request,
        ...
        try {
            ...
            if (jwt == null) {
                ...
                if ("request".equalsIgnoreCase(mapData.get("requestType"))) {
                ...
                ApiResult apiResult = callApiRsspService.ownerChangeEmail(responseTse, jsonObject, request);
// gọi server
...
// trả kết quả
    ...
    @PostMapping("/tse/owner/changeInfo")
    public ResponseEntity ownerChangeInfo(HttpServletResponse response, HttpServletRequest request,
            @RequestBody Map<String, String> mapData) {
        try {
            log.info("request: {}", CommonUtil.convertObjectToString(mapData));
            String jwt = JWTFilter.extractAccessTokenFromCookie(request, response, callApiRsspService);
            if (jwt == null) {
                ...
                if (!StringUtils.isEmpty(mapData.get("fullname"))) {
                    jsonObject.put("fullname", mapData.get("fullname"));
                }
                if (!StringUtils.isEmpty(mapData.get("twoFactorMethod"))) {
                    jsonObject.put("twoFactorMethod", mapData.get("twoFactorMethod"));
                }
                if (!StringUtils.isEmpty(mapData.get("identification"))) {
                    jsonObject.put("identification", mapData.get("identification"));
                    jsonObject.put("identificationType", mapData.get("identificationType"));
                }

                ApiResult apiResult = callApiRsspService.ownerChangeInfo(responseTse, jsonObject, request);
